package com.xpxcoder.xpxDroid.views.gdmap.cluster;
...
import com.xpxcoder.xpxDroid.views.gdmap.cluster.demo.RegionItem;
...
//默认最多会缓存80张图片作为聚合显示元素图片,根据自己显示需求和app使用内存情况,可以修改数量
    ...
    private void addSingleClusterToMap(Cluster cluster) {
        ...
        markerOptions.anchor(0.5f, 0.5f)
                .icon(getBitmapDes(cluster.getMarker(), cluster.getClusterCount())).position(latlng);
        ...
        if(cluster.getClusterCount()==1){
            RegionItem item=(RegionItem) cluster.getClusterItems().get(0);
            marker.setTitle(item.getTitle());
            marker.setSnippet(item.getTitle());
        }
     ...
     * 获取每个聚合点的绘制样式
     */
    private BitmapDescriptor getBitmapDes(Marker marker,int num) {
    ...
    private void updateCluster(Cluster cluster) {
            ...
            marker.setIcon(getBitmapDes(marker,cluster.getClusterCount()));
